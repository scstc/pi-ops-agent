...
## 评测结果(2026-09-11,WSL 纯 CPU,5 个真实运维任务)

任务:磁盘用量 / 服务状态 / 最大日志文件 / 内存 Top5 / 端口占用定位(最难,多轮:查端口→找进程→定性)。

| 模型 | 干净轮次结果 | 质量 |
|---|---|---|
| qwen3.5-2b(默认) | **5/5、5/5**(两轮全过) | port8080 能定位 docker-proxy+PID,但归因浅、偶有小幻觉(建议了不存在的 systemd 单元) |
| qwen3.5-4b | 5/5、4/5(一轮 port8080 300s 超时) | 质量明显更高:容器 IP、双栈监听、连 llama-server 自身 ~5GB 占用都观察到 |

结论:两家都能干活、**都存在偶发超时波动**(多轮长上下文任务在纯 CPU 下受 prompt 重处理制约,实测 ~150 tok/s ingest + ~27 tok/s 生成);差异在**答案深度**:4b 明显扎实,2b 及格线。默认 2b 是内存受限场景取舍,切 4b 一条命令:`pi-ops --model llama-cpp/qwen3.5-4b`。跑分脚本:`tests/eval-ops.sh [模型id]`。

> 排障记录:曾出现"装完默认 2b 实际跑 4b"——systemd 管理器级 `MODEL` 环境变量(临时 A/B 切换用)跨服务重启/重装存活,静默压过安装默认值;install.sh 现已重装时 `unset-environment MODEL` 归位。
