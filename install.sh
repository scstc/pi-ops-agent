...
# 控制台输出语言自适应:UTF-8 终端中文,否则 ASCII 英文(客户现场终端编码不定)
set -euo pipefail
...
die()  { printf '\033[1;31m[install] %s:\033[0m %s\n' "$(msg "失败" "FAIL")" "$(msg "$@")" >&2; exit 1; }

# ---------- 0. 输入消毒:路径会被展开进生成的脚本/JSON,拒绝特殊字符 ----------
for v in PI_OPS_HOME HOME; do
  eval "val=\"\$$v\""
  case "$val" in
    *[!A-Za-z0-9/._~-]*) die "\$$v 含非法字符(仅允许字母数字 / . _ - ~):$val" "invalid chars in \$v (allowed: A-Za-z0-9 / . _ - ~): $val" ;;
  esac
done

# ---------- 1. 产物完整性 ----------
if [ -f "$BUNDLE/MANIFEST.sha256" ]; then
  log "校验 bundle 完整性(sha256)…" "verifying bundle integrity (sha256) ..."
  ( cd "$BUNDLE" && sha256sum -c MANIFEST.sha256 --quiet ) \
    || die "bundle 校验失败,产物在传输中损坏" "bundle verify failed, artifacts corrupted in transfer"
  ...
  [ -z "$extras" ] || die "bundle/ 存在清单外文件(来源不明,拒绝安装;在联网机重跑 fetch-bundle.sh 可重建清单):$(echo "$extras" | tr '\n' ' ')" \
                        "bundle/ has files not in manifest (unknown origin, refused; re-run fetch-bundle.sh on a connected machine to rebuild): $(echo "$extras" | tr '\n' ' ')"
else
  warn "无 MANIFEST.sha256,跳过完整性校验(注:清单只防搬运损坏,信任锚点是备货的联网机)" \
       "no MANIFEST.sha256, integrity check skipped (note: manifest only guards transfer damage; trust anchor is the packing machine)"
fi
...
if node_ge 22 19; then
  ...
  [ -n "$NODE_TAR" ] || die "无可用 node(系统未装且 bundle 缺 node-v22*-linux-x64.tar.xz)" \
                           "no node (not on system and missing node-v22*-linux-x64.tar.xz in bundle)"
  log "离线安装 node:$(basename "$NODE_TAR")" "offline install node: $(basename "$NODE_TAR")"
...
if [ -n "$LLAMA_BIN" ]; then
  log "检测到系统 llama-server:$LLAMA_BIN,复用" "system llama-server found: $LLAMA_BIN, reuse"
else
  LLAMA_TGZ="$(ls "$BUNDLE"/llama-*.tar.gz 2>/dev/null | head -1 || true)"
  [ -n "$LLAMA_TGZ" ] || die "无 llama-server(系统未装且 bundle 缺 llama-*.tar.gz)" \
                            "no llama-server (not on system and missing llama-*.tar.gz in bundle)"
  log "离线安装 llama.cpp:$(basename "$LLAMA_TGZ")" "offline install llama.cpp: $(basename "$LLAMA_TGZ")"
  ...
  [ -n "$LLAMA_BIN" ] || die "llama 压缩包里没找到 llama-server 可执行文件" "llama-server executable not found in tarball"
  chmod +x "$LLAMA_BIN"
fi
...
for g in "$BUNDLE"/*.gguf; do
  base="$(basename "$g")"
  case "$base" in
    *[!A-Za-z0-9._-]*) die "模型文件名含非法字符:$base" "invalid chars in model filename: $base" ;;
  esac
...
if [ -n "$PI_INSTALLED" ] && [ -n "$PI_WANT" ] && [ "$PI_INSTALLED" != "$PI_WANT" ]; then
  log "pi 版本变化:$PI_INSTALLED → $PI_WANT,重新安装" "pi version change: $PI_INSTALLED -> $PI_WANT, reinstalling"
  rm -rf "$PI_OPS_HOME/pi"
  PI_INSTALLED=""
fi
if [ -z "$PI_INSTALLED" ] && [ -f "$BUNDLE/pi-bundle.tar.gz" ]; then
  log "离线安装 pi …" "offline install pi ..."
  ...
  die "无 pi(bundle 缺 pi-bundle.tar.gz)" "no pi (pi-bundle.tar.gz missing in bundle)"
else
  log "pi $PI_INSTALLED 已安装,跳过" "pi $PI_INSTALLED already installed, skip"
fi
PI_CLI="$PI_OPS_HOME/pi/node_modules/.bin/pi"
[ -f "$PI_CLI" ] || die "pi 安装异常:缺 $PI_CLI" "pi install broken: missing $PI_CLI"
...
if command -v ss >/dev/null 2>&1; then
  ...
  warn "无 ss 命令,跳过端口占用检测(假定 $PORT 空闲;若被占服务会反复重启)" \
       "no ss command, port check skipped (assume $PORT free; service will crash-loop if occupied)"
fi
log "llama-server 使用端口:$PORT" "llama-server port: $PORT"

# ---------- 8. pi 全自动配置 ----------
mkdir -p "$PI_DIR/extensions"
backup() { # 滚动备份:保留上一版用户定制
  [ -f "$1" ] || return 0
  cp "$1" "$1.$(date +%Y%m%d%H%M%S).bak" || die "备份失败:$1(磁盘满?)" "backup failed: $1 (disk full?)"
  warn "已备份 $1 → *.bak" "backed up $1 -> *.bak"
}
...
log "写入 $PI_DIR/models.json(provider=llama-cpp → 127.0.0.1:$PORT,带 api-key)" \
    "wrote $PI_DIR/models.json (provider=llama-cpp -> 127.0.0.1:$PORT, with api-key)"
...
# 输出语言自适应(UTF-8 中文 / ASCII 英文)
IS_UTF8_=0
case "\${LC_ALL:-\${LANG:-}}" in *UTF-8*|*utf8*|*UTF8*) IS_UTF8_=1 ;; esac
m_() { if [ "\$IS_UTF8_" = 1 ]; then printf '%s' "\$1"; else printf '%s' "\${2:-\$1}"; fi; }

model_up() { curl -sf -m 2 -H "Authorization: Bearer \$KEY_" "http://127.0.0.1:\$PORT_/v1/models" >/dev/null 2>&1; }
if ! model_up; then
  echo "[pi-ops] \$(m_ "本地模型未运行,启动中…" "local model not running, starting ...")" >&2
  ...
  for i_ in \$(seq 1 "\$t_"); do
    ...
    printf '\r[pi-ops] %s %ss / %ss ' "\$(m_ "模型加载中…" "loading model ...")" "\$i_" "\$t_" >&2
    sleep 1
  done
  if [ "\$ok_" = 1 ]; then
    printf '\r[pi-ops] %s %ss%s\n' "\$(m_ "模型就绪(耗时" "model ready (took")" "\$i_" "\$(m_ "),进入 pi…" "s, entering pi ...")\$(printf '%40s' ' ')" >&2
  else
    printf '\r\033[2K[pi-ops] %s %ss %s\n' "\$(m_ "警告:模型服务" "WARN: model service not ready in")" "\$t_" "\$(m_ "秒未就绪,继续启动 pi(可能连不上模型)" "s, starting pi anyway (model may be unreachable)")" >&2
  fi
fi
touch "\$MARKER_"
...
if [ "$SYS_STATE" = "running" ] || [ "$SYS_STATE" = "degraded" ]; then
  log "systemd --user 可用(状态:$SYS_STATE),安装用户级服务 …" "systemd --user available (state: $SYS_STATE), installing user service ..."
...
EOF
  ...
  if loginctl enable-linger "$USER" 2>/dev/null; then
    log "已启用 linger(SSH 登出后服务保持运行)" "linger enabled (service survives SSH logout)"
  else
    warn "无法启用 linger:登出最后一个会话后服务会停止,需管理员执行:loginctl enable-linger $USER" \
         "cannot enable linger: service stops after last session logs out; admin should run: loginctl enable-linger $USER"
  fi
...
EOF
  ...
  systemctl --user enable --now pi-ops-llama-idle.timer >/dev/null 2>&1 \
    || warn "空闲回收定时器启用失败(模型将常驻;可手动把 bin/idle-stop.sh 挂 cron)" \
            "idle reaper timer enable failed (model stays resident; cron bin/idle-stop.sh manually)"
else
  log "systemd --user 不可用(状态:${SYS_STATE:-无}),nohup 兜底启动" "systemd --user unavailable (state: ${SYS_STATE:-none}), nohup fallback"
  nohup "$RUNNER" > "$PI_OPS_HOME/logs/llama.out" 2>&1 &
  echo $! > "$PI_OPS_HOME/llama.pid"
  warn "注意:nohup 兜底模式在机器重启后需手动执行 $RUNNER(或改用 systemd)" \
       "note: nohup fallback requires manual $RUNNER after reboot (or switch to systemd)"
fi

# ---------- 11. 冒烟 ----------
log "等待 llama-server 就绪(首次加载模型需数十秒)…" "waiting for llama-server (first load takes tens of seconds) ..."
ok=0
...
[ "$ok" = 1 ] || die "llama-server 120s 未就绪(检查端口占用/模型完整性;日志:$PI_OPS_HOME/logs/llama.out 或 journalctl --user -u pi-ops-llama)" \
                    "llama-server not ready in 120s (check port occupancy / model integrity; logs: $PI_OPS_HOME/logs/llama.out or journalctl --user -u pi-ops-llama)"
log "llama-server 就绪 ✓" "llama-server ready OK"

log "冒烟:chat+tools 请求 …" "smoke: chat+tools request ..."
smoke="$(curl -sf -m 60 -H "Authorization: Bearer $LLAMA_KEY" -H 'Content-Type: application/json' \
  ...
  || die "chat 接口冒烟失败(检查 llama 日志与 llama.key)" "chat smoke failed (check llama logs and llama.key)"
log "chat 响应片段:${smoke:0:160}" "chat response head: ${smoke:0:160}"
...
log "冒烟:pi 端到端(headless)…" "smoke: pi end-to-end (headless) ..."
$TO "$PI_OPS_HOME/bin/pi-ops" -p "用一句话自我介绍(你是 pi-ops-agent),然后只回复:就绪" < /dev/null \
  || die "pi 冒烟失败(模型:$DEFAULT_ID;检查 $PI_DIR/models.json 与 llama 日志)" \
         "pi smoke failed (model: $DEFAULT_ID; check $PI_DIR/models.json and llama logs)"

log "冒烟:审批门(headless 下危险命令应被默认拒绝)…" "smoke: approval gate (dangerous command must be denied in headless) ..."
...
if [ -e "$GATE_DIR" ]; then
  log "审批门生效 ✓(危险命令被拒,$GATE_DIR 未被删除)" "approval gate OK (dangerous command denied, $GATE_DIR untouched)"
  rm -rf "$GATE_DIR"
else
  rm -rf "$GATE_DIR" 2>/dev/null || true
  die "审批门未拦截危险命令(headless 下应默认拒绝)——检查扩展加载" \
      "approval gate failed to block dangerous command (should deny in headless) - check extension loading"
fi
...
============================================
 pi-ops-agent install DONE
============================================
 Home        : $PI_OPS_HOME
 Model (def) : $DEFAULT_ID  (llama-server 127.0.0.1:$PORT, with api-key)
 Models      : $(ls "$PI_OPS_HOME/models/" | tr '\n' ' ')
 Usage       : pi-ops                    # interactive TUI
               pi-ops -p "check disk"   # one-shot query
 Switch model: pi-ops --model llama-cpp/qwen3.5-4b
 Service     : systemctl --user status pi-ops-llama
               (no systemd: $PI_OPS_HOME/bin/llama-run &)
 Logs        : $PI_OPS_HOME/logs/llama.out or journalctl --user -u pi-ops-llama
 Open a new shell (or source ~/.bashrc) to get pi-ops in PATH
============================================
EOF
fi